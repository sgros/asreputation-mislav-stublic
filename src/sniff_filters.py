...
logHandler = logging.FileHandler('sniff_sensor.log')
...
def filter_unknown_tld(pkt):
    ...
    if KNOWNTLDS is None:
        KNOWNTLDS = {}
        try:
            f = file('./data/' + KNOWNTLD_FILE, 'r')
        except IOError:
            log.error('Could not open %s' % KNOWNTLD_FILE)
            return False

        for line in f:
            KNOWNTLDS[(line.strip()).lower()] = 1
        f.close()
...
def filter_unknown_query_class(pkt):
    """Search for unknown query class
    """
    for i in _get_question(pkt):
            if i['qclass'] not in ('IN', 'CS', 'CH', 'HS', 'ANY'):
                return True

    return False

def filter_obsolete_query_class(pkt):
    """See if query class is CS, obsolete
    """
...
def filter_obsolete_opcode(pkt):
    """
    Check if send opcode is unknown
    """
    
    if _get_opcode(pkt) is 'IQUERY':
        #log.critical('Query contains unknown code: %s' %  pkt)
        return True

    return False

def filter_port_0(pkt):
    """See if source port is set to zero
    """
    
    if pkt['UDP']['sport'] is '0' or pkt['UDP']['dport'] is '0':
        return True
